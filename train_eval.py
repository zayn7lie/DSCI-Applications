...
def adjust_lr(optimizer, epoch, modellr):
    modellrnew = modellr * 0.1 ** (epoch // 40) # (0.1 ** (epoch // 100)) # 
...
def eval(model, device, test_loader):
    ...
    with torch.no_grad():
        ...
        f1, auc = 0, 0
        threshold = [0.1, 0.1, 1e-2, 1e-2, 1e-2, 1e-3, 1e-2, 1e-2]
        for i in range(8):
            # print(sum_o[i], sum_t[i])
            # print(np.array(sum_o[i] >= , dtype=float))
            temp_f1, temp_auc = f1_score(sum_t[i], np.array(sum_o[i] >= threshold[i], dtype=float)), roc_auc_score(sum_t[i], sum_o[i])
