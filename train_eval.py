...
def eval(model, device, test_loader):
    ...
    with torch.no_grad():
        ...
        f1, auc = 0, 0
        TH = [0.5, 0.4, 0.3, 0.2, 0.1, 0.1, 0.1, 0.1]
        for i in range(8):
            temp_sum_o = np.array(sum_o >= TH[i], dtype=float)
            temp_f1, temp_auc = f1_score(sum_t[i], temp_sum_o), roc_auc_score(sum_t[i], sum_o[i])
